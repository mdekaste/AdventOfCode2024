...
import core.interval.implementations.from
import core.interval.implementations.until
import java.util.*
...
class TreeIntervalMap<K : Comparable<K>, V>(
    ...
    override fun firstEntry(): Map.Entry<Interval.Left.Open<K>, V> {
        val (until, value) = _backingMap.firstEntry()
        val interval = until(until)
        return AbstractMap.SimpleEntry(interval, value)
    }

    override fun lastEntry(): Map.Entry<Interval.Right.Open<K>, V> {
